...
/*
#define Mpu6050_INT_inmode     Port_In_Out(PortC,BIT7,Updown_IN) //输出模式 10M 
#define Mpu6050_INT_input      PCin(7)	 				         //输入的数据
#define Mpu6050_INT_output     PCout(7)	
*/
...
uchar MPU6050_testConnection(void); //功能:	检测MPU6050 是否已经连接 0:有设备 1:无
void  MPU6050_InitGyro_Offset(void);//初始化陀螺仪偏置
float MPU6050_1GValue(void);        //读取加速度计三个轴的总和
//---常用：--------------------------------------------
uchar MPU6050_initialize(void);       //初始化
void MPU6050_GetXYZ(sint * values);   //获取XYZ
